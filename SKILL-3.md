...
- If the contract explicitly documents the behavior as a known trade-off (e.g., IGP's ERC20 pull warning): no broken promise.
- If the behavior is a necessary consequence of the contract's design (e.g., immutable storage, no versioning): no broken promise unless the contract claims versioning.
...
- Caution: "self-inflicted" is not a lazy escape. If the contract presents itself as protecting users from a class of malicious interaction, the victim signing a transaction is not consent to that specific harm. The victim must be able to trivially avoid the harm while still using the contract as intended.
...
- **Medium**: Externally caused harm with no direct theft of principal. Typically griefing (transaction failure, gas waste) that the victim cannot trivially avoid without sacrificing functionality, and the attacker has no profit motive.
...
One summary per finding. No em dashes. Backticks on function names and code references. Do not reference other issue IDs in the reasoning.
...
## Anti-Patterns (What This Session Taught Me)

1. **Assuming deployment without checking the assets table.** If it's not in the assets table, it's not in scope. Stop there.
2. **Calling things duplicates without independent analysis.** Every finding gets its own gate evaluation. If the same root cause appears in two findings, note it in the caveats but evaluate each independently.
...
4. **Excusing custody failures with "the victim could have avoided it."** That's Gate 4 analysis, not a shortcut. Evaluate properly.
5. **Parroting the reviewer's reasoning.** Write your own analysis. The reviewer may be wrong. Verify everything against the code.
...
7. **Using em dashes or forgetting backticks on function names.** Format matters for readability.
